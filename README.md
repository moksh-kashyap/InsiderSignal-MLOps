...
Requires a local MySQL instance and a MYSQL\_ROOT\_PASSWORD environment variable set. Schema is in schema.sql.
...
\## Real debugging log (because it happened)



While standing this up, I hit and diagnosed:

\- Airflow's webserver/scheduler dying silently when their terminal windows closed - fixed by running them detached with nohup
...
\- A GitHub Actions env-var naming mismatch between the workflow and test fixtures

\- register\_model() assuming a models/ directory that does not exist on a fresh CI machine - fixed with os.makedirs(..., exist\_ok=True)
